...
tr1 = Image.load("Sprites/Tracks/Flower Cup/Toad's turnpike/1.png")
tr2 = Image.load("Sprites/Tracks/Flower Cup/Toad's turnpike/2.png")
tr3 = Image.load("Sprites/Tracks/Flower Cup/Toad's turnpike/3.png")
...
while true do
	...
	if race == true  then
		...
		if banana.x <= - 50 then 
			banana.x = 1100
			banana.y = math.random(150,272)
		end

		shell.x = shell.x - 3.5
		if shell.x <= - 60 then 
			shell.x = 750
			shell.y = math.random(150,272)
		end

		star.x = star.x - 4.9
		if star.x <= - 60 then 
			star.x = 900
			star.y = math.random(150,272)
		end

		mushroom.x = mushroom.x - 5
		if mushroom.x <= 70 then
			mushroom.x = 700
			mushroom.y = math.random(150,272)
		end
	---------------------------------------------
	...
	-----------------------------------------------
		...
		if current_lap == 9 and player.x > opponent.x  then	
			...
			file:write(m_timer)
			file:close()
			file = io.open("results/1temp.spl" , "w")
			file:write(m_timer)
			file:close()
			screen:clear()
		...
		if player.y <= 140 - player.height then player.y = 140 - player.height end
		if opponent.y <= 140 - opponent.height then opponent.y = 140 - opponent.height end
