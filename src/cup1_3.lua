...
tr1 = Image.load("Sprites/Tracks/Mushroom Cup/Koopa Troopa Beach/1.png")
tr2 = Image.load("Sprites/Tracks/Mushroom Cup/Koopa Troopa Beach/2.png")
tr3 = Image.load("Sprites/Tracks/Mushroom Cup/Koopa Troopa Beach/3.png")
...
while true do
	...
	if race == true  then
		...
		if banana.x <= - 50 then 
			banana.x = 1100
			banana.y = math.random(118,201)
		end

		shell.x = shell.x - 3.5
		if shell.x <= - 60 then 
			shell.x = 750
			shell.y = math.random(118,201)
		end

		star.x = star.x - 4.9
		if star.x <= - 60 then 
			star.x = 900
			star.y = math.random(118,201)
		end

		mushroom.x = mushroom.x - 5
		if mushroom.x <= 70 then
			mushroom.x = 700
			mushroom.y = math.random(118,201)
		end
	---------------------------------------------
	...
	-----------------------------------------------
		...
		if current_lap == 9 and player.x > opponent.x  then	
			...
			file:write(m_timer)
			file:close()
			file = io.open("results/4temp.spl" , "w")
			file:write(m_timer)
			file:close()
			file = io.open("data/cup1_3p.spl" , "w")
			file:write(3)
			file:close()
			file = io.open("data/cup1_3e.spl" , "w")
			file:write(2)
			file:close()
			screen:clear()
			dofile("cup1r3w.lua")
		...
		if player.y <= 118 - player.height then player.y = 118 - player.height
		elseif player.y >= 201 - player.height then player.y = 201 - player.height end
		if opponent.y <= 118 - opponent.height then opponent.y = 118 - opponent.height
		elseif opponent.y >= 201 - opponent.height then opponent.y = 201 - opponent.height end
	-------------
	...
	if info == true then screen:fontPrint(font,150,180,"3rd Cup Event/4",Color.new(255,0,0)) end
